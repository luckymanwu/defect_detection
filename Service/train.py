...
def train_init(hyp,opt):
    cfg = opt.cfg  #default='cfg/yolov3-tiny.cfg'
    data = opt.data  #default='data/rbc.data'
    epochs = opt.epochs  # 500200 batches at bs 64, 117263 images = 273 epochs   default=2000
    batch_size = opt.batch_size   #default=16
    accumulate = max(round(64 / batch_size), 1)  # accumulate n times before optimizer update (bs 64)  # 4
    weights = opt.weights  # initial training weights  #default='weights/yolov3-tiny.weights'
    ...
    train_path = data_dict['train']    #train=data/train.txt
    test_path = data_dict['valid']      #valid=data/test.txt
    nc = 1 if opt.single_cls else int(data_dict['classes'])  # number of classes  #classes=8
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default=2000)  # 500200 batches at bs 16, 117263 COCO images = 273 epochs
    parser.add_argument('--batch-size', type=int, default=16)  # effective bs = batch_size * accumulate = 16 * 4 = 64
    parser.add_argument('--cfg', type=str, default='cfg/yolov3-tiny.cfg', help='*.cfg path')
    parser.add_argument('--data', type=str, default='data/rbc.data', help='*.data path')
