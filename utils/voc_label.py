...
def convert_annotation(image_id):
    ...
    out_file = open("C:\\Users\\Administrator\\Desktop\\trash\\labels\\%s.txt" % (image_id), 'w', encoding='utf-8')
    # print(out_file.name)
    # 解析xml文件
...
　　　　２．同时对所有的图片文件进行解析和转化，将其对应的bundingbox 以及类别的信息全部解析写到label 文件中去
    ...
    # 包含对应的文件名称
    image_ids = open("C:\\Users\\Administrator\\Desktop\\trash\\ImageSets\\%s.txt" % (image_set)).read().strip().split()
    # 打开对应的2012_train.txt 文件对其进行写入准备
    list_file = open("C:\\Users\\Administrator\\Desktop\\trash\\label\\%s.txt" % (image_set), 'w')
    # 将对应的文件_id以及全路径写进去并换行
    for image_id in image_ids:
        list_file.write("C:\\Users\\Administrator\\Desktop\\trash\\images\\%s.jpg" % (image_id))
